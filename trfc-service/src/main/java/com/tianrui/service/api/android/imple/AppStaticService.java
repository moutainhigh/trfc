...
public class AppStaticService implements IAppStaticService {
							...
							if (StringUtils.isNotBlank(param.getMiningpointid()) && !StringUtils.equals(notice.getMiningpointid(), param.getMiningpointid())) {
								...
								if (miningpoint != null) {
									if (StringUtils.equals(miningpoint.getIsvalid(), Constant.ONE_STRING)) {
										...
										result.setErrorCode(ErrorCode.NOTICE_ISVALID);
										return result;
									}
								} else {
									result.setErrorCode(ErrorCode.NOTICE_MININGPOINT);
									return result;
								}
	...
	public AppResult getMiningList(AppMiningParam appMiningParam) {
		...
		if (appMiningParam != null && StringUtils.isNotBlank(appMiningParam.getMaterId())
				&&StringUtils.isNotBlank(appMiningParam.getUserId())) {
			List<MiningpointDbSettingResp> list = miningpointDbSettingMapper.selectByMaterialid(appMiningParam.getMaterId(),appMiningParam.getUserId());
			result.setData(list);
			result.setErrorCode(ErrorCode.SYSTEM_SUCCESS);
		} else {
