...
@Service
public class SystemUserclientService implements ISystemUserclientService {
	@Autowired
	...
	public Result apiLogin(UserReq req) throws Exception {
		...
		if (req!=null && StringUtils.isNotBlank(req.getPswd()) && StringUtils.isNotBlank(req.getAccount()) && StringUtils.isNotBlank(req.getSubSystemCode())) {
			...
			//获取当前用户
			List<SystemUser> list = userclientMapper.selectByCondition(query);
	...
	public Result editUser(SystemUserSaveReq req) throws Exception {
		...
		if ( validateReq(req,2) ) {
			//用户需要能查询到
			SystemUser db=userclientMapper.selectByPrimaryKey(req.getId());
			if ( db !=null ) {
				//保存数据
...
//				update.setAccount(req.getAccount());
				...
				userclientMapper.updateByPrimaryKeySelective(update);
				rs.setErrorCode(ErrorCode.SYSTEM_SUCCESS);
			}else{
				rs.setErrorCode(ErrorCode.SYSTEM_USER_ERROR1);
			}
	...
	public Result delUser(SystemUserQueryReq req) throws Exception {
		...
		if ( req !=null && StringUtils.isNotBlank(req.getId()) ) {
			...
			if (db !=null) {
				//TODO关联关系删除
				userclientMapper.deleteByPrimaryKey(req.getId());
				rs.setErrorCode(ErrorCode.SYSTEM_SUCCESS);
			}else{
				rs.setErrorCode(ErrorCode.SYSTEM_USER_ERROR1);
			}
	...
	public Result resetPwd(SystemUserSaveReq req) {
		...
		if(req != null && StringUtils.isNotBlank(req.getId())
			...
			bean.setModifier(req.getId());
			bean.setModifytime(System.currentTimeMillis());
			if(userclientMapper.updateByPrimaryKeySelective(bean) == 1){
				result.setErrorCode(ErrorCode.SYSTEM_SUCCESS);
			}else{
				result.setErrorCode(ErrorCode.OPERATE_ERROR);
			}
...
//                                }
	...
	public Result pages(SystemUserQueryReq req) throws Exception {
		...
		if (req != null) {
			page = new PaginationVO<SystemUserResp>();
			long count = userclientMapper.countUserClient(req);
			if (count > 0) {
				req.setStart((req.getPageNo()-1)*req.getPageSize());
				req.setLimit(req.getPageSize());
				List<SystemUser> list = userclientMapper.selectUserClient(req);
				page.setList(copySystemUserBeanList2RespList(list));
			}
			page.setPageNo(req.getPageNo());
			page.setPageSize(req.getPageSize());
	...
	public Result selectAccountUser(String phone) throws Exception {
		Result rs =Result.getSuccessResult();
		List<SystemUser> list =userclientMapper.selectAccountUser(phone);
