...
import com.tianrui.service.mapper.system.auth.SubsystemMapper;
...
/**
 * 子系统管理
  * <p>Title:SubsystemService </p>
  * <p>Description:TODO </p>
  * <p>Company: </p> 
  * @author   yangbobo
  * @date   2017年11月10日 上午11:37:40
 */
@Service
public class SubsystemService implements ISubsystemService{
	...
	public Result page(SystemMenuQueryReq req) throws Exception {
		...
		if(req != null){
			...
			long count = subsystemMapper.countByCondition(req);
			if(count > 0){
//				req.setStart((req.getPageNo()-1)*req.getPageSize());
//				req.setLimit(req.getPageSize());
				List<SystemMenu> list = subsystemMapper.selectByCondition(req);
	...
	public Result getTreeData() throws Exception {
		...
		queryReq.setStart(0);
		List<SystemMenu> list = subsystemMapper.selectByCondition(queryReq);
	...
	public Result detail(SystemMenuQueryReq req) throws Exception {
		...
		if( req !=null && StringUtils.isNotBlank(req.getId())){
			SystemMenu db=subsystemMapper.selectByPrimaryKey(req.getId());
			if( db !=null ){
	...
	public Result addMenu(SystemMenuSaveReq req) throws Exception {
		...
		if(req!=null){
			...
			List<SystemMenu> _list =subsystemMapper.selectSelective(query);
			...
			List<SystemMenu> list =subsystemMapper.selectSelective(query);
			if( list!=null && list.size()>0){
				rs.setErrorCode(ErrorCode.SYSTEM_MENU_ERROR8);
				return rs;
			}
			//获取深度
			SystemMenu sm = subsystemMapper.selectByPrimaryKey(req.getRoleid());
			if(sm!=null){
			...
			if(subsystemMapper.insertSelective(bean)>0){
				rs.setErrorCode(ErrorCode.SYSTEM_SUCCESS);
				rs.setData(bean);
			}else{
	...
	public Result editMenu(SystemMenuSaveReq req) throws Exception {
		...
		if(req!=null && StringUtils.isNotBlank(req.getId())){
			...
			bean.setModifier(req.getCurrUId());
			bean.setModifytime(System.currentTimeMillis());
			if(subsystemMapper.updateByPrimaryKeySelective(bean)>0){
	...
	public Result delMenu(SystemMenuQueryReq req) throws Exception {
		...
		if( req !=null && StringUtils.isNotBlank(req.getId()) ){
			...
			if(db !=null){
				//TODO关联关系删除
				if(subsystemMapper.deleteByPrimaryKey(req.getId())>0){
					rs.setErrorCode(ErrorCode.SYSTEM_SUCCESS);
				}else{
					rs.setErrorCode(ErrorCode.OPERATE_ERROR);
				}
	...
	public Result findMenuByUserId(String uId) throws Exception {
		...
		if(StringUtils.isNotBlank(uId)){
			List<SystemMenu> list = subsystemMapper.selectMenuByUserRoleId(uId);
