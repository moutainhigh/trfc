...
//				console.log(data);
				...
				for(var i=0;i<list.length;i++){
					...
					var tr=$('<tr><td>'+(i+1)+'</td><td>'+role.code+'</td><td>'+role.name+'</td><td>'+roleType+'</td><td><input type="checkbox" disabled id="isvalid'+i+'">'+
							...
							'<span class="select"><a data-toggle="modal" data-target="#select"><i class="iconfont" id="selectRole"  style="margin: 0 5px;" data-toggle="tooltip" data-placement="left" title="权限查看">&#xe651;</i></a></span>'+
...
//		console.log(params);
	...
	function selectRole(roleData){
		var url=URL.selectRole;
		var param={roleid:roleData.id};
			...
			if(result.code=='000000'){
				...
				if(datas!=undefined){
					for(var i=0;i<datas.length;i++){
						var obj = datas[i];
						$('<li class="nNone">'+obj.menuName+'</li>').appendTo('.lookRole');
					}
				}else{
					for(var i=0;i<4;i++){
