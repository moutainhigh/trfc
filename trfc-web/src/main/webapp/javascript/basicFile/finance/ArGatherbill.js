;(function($, win){
	...
	var URL={
			pageUrl:"/trfc/arGatherbill/page",
	...
	function queryData(pageNo){
		...
		//var saleOrgName=$('#branch option:checked').text();
		var billno=$('#code').val();billno=$.trim(billno);
		var pageNo=pageNo;
...
//				console.log(data);
				...
				for(var i=0;i<list.length;i++){
					var charge=list[i];
//					console.log(charge);
					var status=charge.approvestatus;
					switch(status){
						case '0':
							status='未审核';
							break;
					...
					var approvedated=new Date(charge.approvedated).format("yyyy-MM-dd hh:mm:ss");
					if(!charge.approvedated){
						approvedated='';
					}
					var approver=charge.approver;
					if(!charge.approver){
						approver='';
					}
					var money = charge.money;
					if (charge.money != null) {
						money = parseFloat(charge.money).toFixed(2);
					}
					var localMoney = charge.localMoney;
