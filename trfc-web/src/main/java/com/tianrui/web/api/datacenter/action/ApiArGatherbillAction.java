 ...
 */
@Controller
@RequestMapping("api/dc/arGatherbill")
public class ApiArGatherbillAction {
	private Logger log=LoggerFactory.getLogger(ApiArGatherbillAction.class);
	
	@Autowired
	IArGatherbillService arGatherbillService;
	...
	public ApiResult getLastUTC(ApiParam<BaseReq> req){
		...
		try {
			rs = arGatherbillService.findMaxUtc(baseReq);
		} catch (Exception e) {
	...
	public ApiResult updateData(ApiParam<List<JSONObject>> req){
		...
		try {
			rs = arGatherbillService.updateDataWithDC(list);
		} catch (Exception e) {
			rs.setErrorCode(ErrorCode.SYSTEM_ERROR);
			log.error(e.getMessage(),e);
		}
